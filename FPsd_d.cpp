...
void solve(){
	...
	forn(i,n){
		forn(j,n){
			dist[i][j] = abs(x[i]-x[j])*abs(x[i]-x[j]) + abs(y[i]-y[j])*abs(y[i]-y[j]);
			dist[j][i] = dist[i][j];
		}
	}
	...
	forn(i, n){
		int u = -1;
		forn(j, n){
			if(!vis[j] && (u == -1 || min_w[j] < min_w[u])) {
				u = j;
			}
		}
		
		if(min_w[u] == LLINF) break;
		
		vis[u] = true;
		
		if(parent[u] != -1){
			mst_adj[u].pb(parent[u]);
			mst_adj[parent[u]].pb(u);
		}
		
		forn(v, n){
			if(!vis[v] && dist[u][v] < min_w[v]){
				min_w[v] = dist[u][v];
				parent[v] = u;
			}
		}
	...
	forn(i, n){
		if(parent[i] != -1){
			mst_weights.pb(min_w[i]);
		}
	}
	
	sort(all(mst_weights));
	ll sz = mst_weights.size();
	forn(i,q){
		ll k;
		cin >> k;
		cout <<1 + sz - ( upper_bound(mst_weights.begin(), mst_weights.end(), k) - mst_weights.begin() ) << nl;
	}
