...
void solve(){
    ...
    forn(i, n){
        string r;
        cin >> r;
        Node* region_node = new Node(r);
        db[r] = region_node;
        regions.pb(region_node);
        ...
        if(sz > 0){
            for(int j = 0; j < sz; j++){
                locals[j]->next = locals[(j + 1) % sz];
                locals[(j + 1) % sz]->prev = locals[j];
            }
            region_node->bridge = locals[0];
            locals[0]->bridge = region_node;
        }
    }
    
    ll r_sz = regions.size();
    if(r_sz > 0){
        for(int i = 0; i < r_sz; i++){
            regions[i]->next = regions[(i + 1) % r_sz];
            regions[(i + 1) % r_sz]->prev = regions[i];
        }
    }
    ...
    forn(qq, q){
        ...
        if(db.find(a) == db.end() || db.find(b) == db.end()){
            cout << "Signal lost" << nl;
            continue; 
        }
        ...
        while(!dq.empty()){
            ...
            vector<Node*> neighbors = {now->next, now->prev, now->bridge};
            for(Node* nxt : neighbors){
                if(nxt != NULL && dist.find(nxt) == dist.end()){
                    dist[nxt] = d + 1;
                    dq.pb(nxt);
                }
            }
        ...
        if(found && dist[target] <= c){
            cout << "Connected on " << dist[target] << " second" << nl;
        } else {
            cout << "Signal lost" << nl;
        }
    }
